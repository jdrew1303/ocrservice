(function() {
  ...
  module.exports = Recognizer = (function(superClass) {
    ...
    Recognizer.prototype.run = function() {
      var im;
    ...
    Recognizer.prototype.appendBarcodes = function(item) {
      ...
      if (code.indexOf('http://') === -1) {
        ...
        if (typeof this.barcodesHash[code] === 'undefined') {
          ...
          return this.barcodes.push(code);
        }
      }
    };

    Recognizer.prototype.getBarcode = function(item) {
      var codes, cropped, imagecodes, j, k, len, len1, r;
      ...
      if (this.barcodes.length === 0) {
        r = item.rect;
        cropped = this.barcode_image.clone();
    ...
    Recognizer.prototype.barcodeOriginalBarDecode = function(cb) {
      var child, codes, me;
      me = this;
      codes = [];
      child = spawn('bardecode', [this.fileName]);
      child.stdout.on('data', function(data) {
        var codeLines, codeparts, j, len, line, results;
        ...
        for (j = 0, len = codeLines.length; j < len; j++) {
          ...
          if (codeparts.length > 2) {
            if (codeparts[2] === 'code25i') {
              ...
              results.push(codes.push(codeparts[0].substring(0, codeparts[0].length - 1)));
            } else {
              debug('found code', codeparts[0]);
              results.push(codes.push(codeparts[0]));
            }
          } else {
            results.push(void 0);
          }
        ...
        if (me.barcodes.length === 0) {
          bfn = function(codes) {
            ...
            if (me.barcodes.length === 0) {
              fn = function(codes) {
                me.barcodes = codes;
                return me.processList(index + 1);
              };
              return me.barcodeOriginal(fn.bind(this));
            } else {
              return me.processList(index + 1);
            }
          };
          return me.barcodeOriginalBarDecode(bfn.bind(this));
        } else {
          return me.processList(index + 1);
        }

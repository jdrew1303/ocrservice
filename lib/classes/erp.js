(function() {
  ...
  module.exports = ERP = (function(superClass) {
    ...
    ERP.prototype.setErp = function() {
      var opt;
      debug('set erp connect', this.options.key);
      opt = {
        autoConnect: false
      };
      this.erp = client(this.url, opt);
      debug('start', 'set up erp io ' + this.url + '' + ' ' + this.options.key);
      ...
      this.erp.on('fastaccess', (function(_this) {
        return function(data) {
          return _this.onFastAccess(_this.erp, data);
        };
      })(this));
    ...
    ERP.prototype.onConnect = function() {
      debug('erp connect', this.options.key);
      return this.emit('connect');
    };

    ERP.prototype.onDisconnect = function() {
      this.emit('disconnect', this.options.key);
    ...
    ERP.prototype.onLoginError = function(socket, msg) {
      this.sid = '';
      debug('erp', msg);
      return this.emit('loginError', msg);
    };

    ERP.prototype.onFastAccess = function(socket, list) {
      debug('erp', 'on fastaccess');
      return this.emit('fastaccess', list);
    };
    ...
    ERP.prototype.fastaccess = function() {
      if (typeof this.erp === 'object' && this.erp.connected === true) {
        return this.erp.emit('fastaccess', {});
      } else {
        return this.emit('error', 'not connected');
      }
    };

(function() {
  ...
  module.exports = Watcher = (function(superClass) {
    ...
    Watcher.prototype.onDiscoveryFound = function(data, remote) {
      var ref;
    ...
    Watcher.prototype.setIoConnectTimer = function() {
    ...
    Watcher.prototype.setIO = function() {
      var opt;
      opt = {
        autoConnect: false
      };
      debug('watcher dispatcher', this.url);
    ...
    Watcher.prototype.onDiscoveryTimout = function() {
    ...
    Watcher.prototype.recognizeBoxes = function(res, codes) {
      ...
      if (!this.run) {
        ...
        if (codes.length === 0) {
          ...
          return fs.rename(file, path.join(me.pathName, 'nocode', name + path.extname(file)), function(err) {
            if (err) {
              console.trace(err);
              return me.emit('error', err);
            } else {

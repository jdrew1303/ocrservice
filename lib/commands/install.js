(function() {
  ...
  module.exports = Install = (function(superClass) {
    ...
    Install.prototype.linuxInstallServiceFile = function() {
      var me;
      me = this;
      return fs.writeFile('/' + path.join('etc', 'systemd', 'system', me.options.servicename + '.service'), servicefiletext, function(err) {
        if (err) {
          throw err;
        } else {
    ...
    Install.prototype.linuxInstallSysconfig = function() {
      var me;
      me = this;
      return fs.writeFile('/' + path.join('etc', 'sysconfig', me.options.servicename), envcontent.join("\n"), function(err) {
        if (err) {
          throw err;
        } else {

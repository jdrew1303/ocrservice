  ...
  module.exports = CBcOcr = (function(superClass) {
    ...
    CBcOcr.prototype.bardecode = function() {
      ...
      if (this.usebardecode && this.codes.length === 0) {
        ...
        bd.on('close', (function(_this) {
          return function(code, signal) {
            return _this.onBarDecodeClose(code, signal);
          };
        })(this));
    ...
    CBcOcr.prototype.onNCRegonizer = function(adrs, codes) {
      if (codes.length === 0) {
      ...
      if (this.codes.length === 0) {
        ...
        this.recognizer.on('boxes', (function(_this) {
          return function(adrs, codes) {
            return _this.onNCRegonizer(adrs, codes);
          };
        })(this));
    ...
    CBcOcr.prototype.onZBarImageClose = function(code, signal) {
    ...
    CBcOcr.prototype.onZBarImage = function(data) {
      ...
      for (i = 0, len = lines.length; i < len; i++) {
        line = lines[i];
        if (line.indexOf(':') > 0) {
          results.push(this.codes.push(this.getZBCode(line)));
        }
      }
      return results;
    };
    ...
    CBcOcr.prototype.onBarDecodeClose = function(code, signal) {
      return this.tesseract();
    };
    ...
    CBcOcr.prototype.onBarDecode = function(data) {
      ...
      for (i = 0, len = lines.length; i < len; i++) {
        ...
        if (line.indexOf('type') > 0) {
          results.push(this.codes.push(this.getBDCode(line)));
        }
      }
      return results;
    };
    ...
    CBcOcr.prototype.onNoCode = function() {
      ...
      moveFile(this.filename, path.join(this.noCodePath, name + path.extname(this.filename)), function(err) {
        return process.exit();
      });
      console.log('nc');
      return console.timeEnd('elapsed');
    };

    CBcOcr.prototype.onNoAddress = function() {
      var box, item, name;
      item = this.adressObject;
      ...
      item.box = [box];
      process.send(item);
      name = (this.codes.join('.')).replace(/[^0-9a-z\.]/gi, '') + ".NA";
      moveFile(this.filename, path.join(this.noAddressPath, name + path.extname(this.filename)), function(err) {
        return process.exit();
      });
      console.log('na');
      return console.timeEnd('elapsed');
    ...
    CBcOcr.prototype.onNoBox = function() {
      ...
      box = {
        ...
        regiogruppe: 'Standardbriefsendungen',
        bereich: 'NT',
        plz: this.adressObject.zipCode,
        ort: this.adressObject.town,
        ortsteil: this.adressObject.district,
        hnvon: '',
        hnbis: '',
        gerade: '',
        ungerade: '',
        strasse: this.adressObject.street
      };
      item.box = [box];
      process.send(item);
      name = (this.codes.join('.')).replace(/[^0-9a-z\.]/gi, '') + ".NT";
      moveFile(this.filename, path.join(this.noAddressPath, name + path.extname(this.filename)), function(err) {
        return process.exit();
      });
      console.log('nt');
    ...
    CBcOcr.prototype.onGood = function() {
      ...
      item.box = [this.box];
      process.send(item);
      name = (this.codes.join('.')).replace(/[^0-9a-z\.]/gi, '');
      moveFile(this.filename, path.join(this.goodPath, name + path.extname(this.filename)), function(err) {
        return process.exit();
      });
      console.log('good');
      return console.timeEnd('elapsed');
    ...
    CBcOcr.prototype.onMoreBoxes = function() {
      ...
      console.log('unclear');
      name = (this.codes.join('.')).replace(/[^0-9a-z\.]/gi, '');
      moveFile(this.filename, path.join(this.badPath, name + path.extname(this.filename)), function(err) {
        return process.exit();
      });
